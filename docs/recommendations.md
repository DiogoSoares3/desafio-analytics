...
that comes straight out of the model behind the **Adventure Works — Sales** dashboard
(`bi/dashboards/adventure_works_sales.yaml`) or the EDA notebook (`notebooks/eda.ipynb`), and every one
of those numbers reconciles to the audited 2011 gross-sales figure ($12,646,112.16) that the finance
team already signed off on. You can open the dashboard, apply the same filters, and get the same
numbers — that is the whole point of building it this way. The recommendations are ordered by expected
commercial impact, highest first.
...
**$1,061.45** for online — a **~20x** gap (`bi/charts/hero_promotion_impact.yaml` and the P4-01 hero
KPIs, cross-checked against the channel-mix chart in `notebooks/eda.ipynb`). Any plan to shift spend
...
(90,268 of 274,914) — the highest revenue-per-unit category by a wide margin (`bi/charts/question_a_orders_qty_value.yaml`,
sliceable by product/category, and the product-mix chart in `notebooks/eda.ipynb`). Components,
...
The single largest customer, **Brakes and Gears**, alone drove **$882,276.4966** in gross revenue
(`bi/charts/question_c_top10_customers.yaml`); the top city, **Toronto**, drove **$4,498,883.7327**
(`bi/charts/question_d_top5_cities.yaml`), roughly 27% more than the second-ranked city, London
($2,754,814). Action: assign a named commercial owner to each of the top 10 customers and top 5 cities
...
a distant but clear second at **$16,441,130** (`bi/charts/question_b_top_products_by_aov.yaml` and
`notebooks/eda.ipynb`'s geography-distribution chart, both filterable by country/state/city). Action:
...
This is the direct answer to your skepticism, Silvana. The Promotion-Impact hero KPI on the dashboard
(`bi/charts/hero_promotion_impact.yaml`, reconciling `notebooks/eda.ipynb`'s independent EDA figure
exactly) shows **3,515** online orders tagged "On Promotion" drove **$6,361,828.95** in gross revenue —
about **5.8%** of total revenue, a real and material number, so "spend on promotions" is not wrong on
its face. But the model also shows the actual dollar `discount_amount` — **$527,507.91** total — sits
**entirely on the reseller/store channel** (60,919 order lines) and is **structurally zero** on every
one of the 60,398 online order lines, including all 3,515 "On Promotion" ones
(`notebooks/eda.ipynb`, promotion/discount-impact section). In other words, the online "Promotion" tag
in this data is a *reason code* (why the customer says they bought), not a price cut — the top product
under that reason, **Water Bottle - 30 oz.** (546 units, `bi/charts/question_f_top_product_promotion.yaml`),
sold at full margin. The actual price discounting happens on reseller deals, a completely different
...
Every number in this document is reproducible from `data/adventureworks.duckdb` by anyone on your team,
end to end, offline — the same $110,373,889.31 total revenue, 31,465 orders, and $3,491.07 average order
value the CEO's audit reconciles to $12,646,112.16 for 2011 alone. That reproducibility is the answer to
"I've been burned by data-driven promises before": this is not a one-time PowerPoint, it is a rebuildable
model with tests attached (`just build`, PASS=138). Action: adopt the "Adventure Works — Sales" dashboard
